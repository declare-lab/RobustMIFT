...
@registry.register_task("commonsensevqa")
class CommonsenseVQATask(BaseTask):
    ...
    def valid_step(self, model, samples):
        ...
        for uid, gen_txt, pred, cand in zip(unique_id, gen_res, ranks, candidates):
            option = eval(cand)
            result.append({"uid": uid, 'gen_txt': gen_txt, 'pred': option[pred[0].item()]})
        return result
    ...
    def _report_metrics(self, eval_result_file):
        ...
        for res in eval_result:
            uid, gen_txt, pred = res["uid"], res["gen_txt"], res["pred"]
            corr += (1 if pred == self.targets[uid] else 0)
