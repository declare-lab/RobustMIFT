...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def __init__(
        ...
        # ).input_ids[0]
        self.pad_token_id = self.llm_tokenizer(
    ...
    def predict_class(
        ...
        if True:
            ...
            for i in range(samples["image"].size(0)):
                ...
                this_result = self._predict_class(this_sample, eval(candidates[i]), n_segments)
    ...
    def _predict_class(
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            this_input_tokens_ids = text_input_tokens.input_ids.repeat_interleave(n_cands, dim=0)
            this_input_tokens_atts = text_input_tokens.attention_mask.repeat_interleave(n_cands, dim=0)


            this_output_tokens_ids = this_output_tokens.input_ids
            this_output_tokens_atts = this_output_tokens.attention_mask
            ...
            inputs_embeds = self.llm_model.get_input_embeddings()(this_llm_input_ids)
            inputs_embeds = torch.cat([inputs_llm.repeat_interleave(n_cands, dim=0), inputs_embeds], dim=1)
            attention_mask = torch.cat([atts_llm.repeat_interleave(n_cands, dim=0), this_llm_atts], dim=1)
            ...
            this_targets = torch.cat([empty_targets.repeat_interleave(n_cands, dim=0), this_targets], dim=1)
