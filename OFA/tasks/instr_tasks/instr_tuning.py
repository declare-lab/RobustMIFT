...
from data.instr_data.instrtuning_dataset import InstrTuningDataset
...
@dataclass
class InstrTuningConfig(OFAConfig):
...
@register_task("instr_tuning", dataclass=InstrTuningConfig)
class InstrTuningTask(OFATask):
    def __init__(self, cfg: InstrTuningConfig, src_dict, tgt_dict):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if split == 'train':
            ...
            dataset = FileDataset(file_path, self.cfg.selected_cols, shuffle=True)
        else:
            file_path = paths[-1]
            dataset = None
        
            
        self.datasets[split] = InstrTuningDataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
