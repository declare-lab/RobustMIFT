...
from data.instr_data.commonsensevqa_dataset import CommonSenseVQAInstrDataset, CommonSenseVQAMCInstrDataset
...
@dataclass
class CommonsenseVQAConfig(OFAConfig):
    ...
    eval_rouge: bool = field(
        default=True, metadata={"help": "evaluation with ROUGE-L scores"}
    )
...
@register_task("commonsense_vqa", dataclass=CommonsenseVQAConfig)
class CommonsenseVQATask(OFATask):
    def __init__(self, cfg: CommonsenseVQAConfig, src_dict, tgt_dict):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = CommonSenseVQAInstrDataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
    ...
    def valid_step(self, sample, model, criterion):
        ...
        if self.cfg.eval_bleu or self.cfg.eval_cider:
            ...
            if self.cfg.eval_rouge:
                scores = self.rscore(hyps, refs)
                logging_output["_rougeL_score_sum"] = scores.sum()
                logging_output["_rougeL_cnt"] = scores.size() 
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        if self.cfg.eval_rouge:
            def compute_rouge(meters):
                rouge = meters["_rougeL_score_sum"].sum() / meters["_rougeL_cnt"].sum()
                rouge = rouge if isinstance(rouge, float) else rouge.item()
                return round(rouge, 3)        
            
            if sum_logs("_rouge_cnt") > 0:
                metrics.log_scalar("_rougeL_score_sum", sum_logs("_rougeL_score_sum"))
                metrics.log_scalar("_rougeL_cnt", sum_logs("_rougeL_cnt"))
                metrics.log_derived("rougeL", compute_rouge)
...
@register_task("commonsense_vqa_mc", dataclass=CommonsenseVQAConfig)
class CommonsenseVQAMCTask(CommonsenseVQATask):
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        paths = self.cfg.data.split(',')
        assert len(paths) > 0
        if split == 'train':
        ...
        self.datasets[split] = CommonSenseVQAMCInstrDataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
