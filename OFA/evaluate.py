...
def main(cfg: DictConfig, **kwargs):
    ...
    for sample in progress:
        ...
        if scores and isinstance(scores[0], tuple):
            if len(scores) == 1:
                ...
                item_sum, item_cnt = torch.zeros(len(scores)).cuda(), torch.zeros(len(scores)).cuda()
                for i, s in enumerate(scores):
                    item_sum[i] += s[0]
                    item_cnt[i] += s[1]
                    if len(score_list) < i + 1:
                        score_list.append([item_sum[i], item_cnt[i]])
                    else:
                        score_list[i][0] += item_sum[i]
                        score_list[i][1] += item_cnt[i]
        else:
    ...
    if len(score_list) > 0:
        for s in score_list:
            merge_results(task, cfg, logger, s[1], s[0], results)
    else:
