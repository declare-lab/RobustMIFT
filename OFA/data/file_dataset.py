...
class FileDataset:
    def __init__(self, file_path, selected_col_ids=None, dtypes=None, separator="\t", cached_index=False, shuffle=True):
        ...
        if dtypes is None:
            # default to str
            self.dtypes = [str for _ in self.selected_col_ids]
        else:
        ...
        self._reader = self._get_tsv_reader()
    ...
    def _seek(self, offset=0):
        ...
        # no need for a lineid2offset transformation
        self._reader.iloc[self.start_pos + offset]
    ...
    def __getitem__(self, index):
        if self.data_cnt == self.row_count:
            ...
            self.data_cnt = 0
            self._reader = self._get_tsv_reader()
        sample = self._reader.iloc[self.start_pos + index]
        self.data_cnt += 1
        column_l = [dtype(sample[col_id]) for col_id, dtype in zip(self.selected_col_ids, self.dtypes)]
        return column_l