...
class CommonSenseVQAInstrDataset(OFADataset):
    def __init__(
        self,
        split,
        dataset,
    ...
    def __getitem__(self, index):
        ...
        uniq_id, image, prompt, target = item

        image = os.path.join('../../../MultiInstruct', image)
        
        image_txt = re.sub('\.(jpg|png|jpeg)', '.txt', image)
        assert image_txt != image
        ...
        src_item = self.encode_text(' {}'.format(prompt))
        src_item = src_item[:self.max_src_length - 2]
...
class CommonSenseVQAMCInstrDataset(CommonSenseVQAInstrDataset):
    def __getitem__(self, index):
        item = self.dataset[index]
        uniq_id, image, prompt, target = item

        image = os.path.join('../../../MultiInstruct', image)
        # with open(image.replace('png','txt')) as f:
        with open(image[:-3] + 'txt') as f:
            img_str = f.read()
            image = Image.open(BytesIO(base64.b64decode(img_str)))
        patch_image = self.patch_resize_transform(image)
        patch_mask = torch.tensor([True])
        ...
        src_item = self.encode_text(' {}'.format(prompt))
        src_item = torch.cat([self.bos_item, src_item[:self.max_src_length-2], self.eos_item])
