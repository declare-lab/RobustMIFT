...
def collate(samples, pad_idx, eos_idx):
    ...
    options = None
    if samples[0].get("option", None) is not None:
        options = [ast.literal_eval(sample["option"]) for sample in samples]
...
class GroundedVQAMCInstrDataset(OFADataset):
    def __init__(
        self,
        split,
        dataset,
    ...
    def __getitem__(self, index):
        ...
        uniq_id, image, option, prompt, target = item
