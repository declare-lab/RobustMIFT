...
class OFADataset(FairseqDataset):
    ...
    def encode_text(self, text, dic='src', length=None, append_bos=False, append_eos=False, use_bpe=True):
        if dic == 'src':
            enc_dict = self.src_dict
        elif dic == 'tgt':
            enc_dict = self.tgt_dict

        s = enc_dict.encode_line(
